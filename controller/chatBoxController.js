...
module.exports.chatBox = async function (req, res, next) {
  try {
    let allUsers;
    if (req.user) {
      ...
      allUsers = await User.find({ _id: { $in: arrayOfUsers } }).sort(
        "-updatedAt"
      );
    }
...
module.exports.deleteMessage = async function (req, res) {
  try {
    ...
    let user = await User.findOne({ email: chatRoom1.withUserEmail }).populate(
      "chatRoomList"
    );
    console.log(user);

    let chatRoom2 = await user.chatRoomList.find(
      (room) => room.withUserEmail == req.user.email
    );
